 ...
 * @author Itamar Jr
 */
public class PorcentagemLancheViagensController implements Initializable {
    ...
    public PorcentagemLancheViagensController() {
    ...
    public void calcularPorcentagem(ActionEvent event) {
        ...
        if (txtHSaida.getText().length() != 0) {
            try {
                ...
                porcentagem = fachadaGerente.calculaPorcentagemLanchePorDatasEViagens(txtHSaida.getText(), txtOrigem.getText(), txtDestino.getText(), dataInicio, dataFinal);
            ...
            try {
                porcentagem = fachadaGerente.calculaPorcentagemLanchePorDatasEDestino(txtOrigem.getText(), txtDestino.getText(), dataInicio, dataFinal);
                txtPorcentagem.setText(porcentagem + " %");
            ...
            try {
                porcentagem = fachadaGerente.calculaPorcentagemLanchePorDatas(dataInicio, dataFinal);
                txtPorcentagem.setText(porcentagem + " %");
            } catch (IntervaloDeDatasInvalidoException ex) {
                ex.printStackTrace();
                alertaErro.setContentText(ex.getMessage());
